# my_Ultralytics YOLO 🚀, AGPL-3.0 license
...
from my_ultralytics.cfg import TASK2DATA, get_cfg, get_save_dir
from my_ultralytics.hub.utils import HUB_WEB_ROOT
from my_ultralytics.nn.tasks import attempt_load_one_weight, guess_model_task, nn, yaml_model_load
from my_ultralytics.utils import ASSETS, DEFAULT_CFG_DICT, LOGGER, RANK, SETTINGS, callbacks, checks, emojis, yaml_load
...
class Model(nn.Module):
    ...
    loaded from local files, my_Ultralytics HUB, or Triton Server. The class is designed to be flexible and
    ...
    Args:
        model (Union[str, Path], optional): Path or name of the model to load or create. This can be a local file
            path, a model name from my_Ultralytics HUB, or a Triton Server model. Defaults to 'yolov8n.pt'.
    ...
    Attributes:
        ...
        session (HUBTrainingSession): The my_Ultralytics HUB session, if applicable.
    ...
    Methods:
        ...
        _get_hub_session: Retrieves or creates an my_Ultralytics HUB session.
        is_triton_model: Checks if a model is a Triton Server model.
        is_hub_model: Checks if a model is an my_Ultralytics HUB model.
    ...
    def __init__(
        ...
        sources, including local files, my_Ultralytics HUB models, and Triton Server models. The method initializes several
        important attributes of the model and prepares it for operations like training, prediction, or export.

        Args:
            model (Union[str, Path], optional): The path or model file to load or create. This can be a local
                file path, a model name from my_Ultralytics HUB, or a Triton Server model. Defaults to 'yolov8n.pt'.
        ...
        self.cfg = None  # if loaded from *.yamlmodel: Union[str, Path] = "yolov8n.pt",
        self.ckpt_path = None
        ...
        # Check if my_Ultralytics HUB model from https://hub.my_ultralytics.com
        ...
        if Path(model).suffix in (".yaml", ".yml"):
            #修改的
            self._new(model, task=task, verbose=verbose, device=device)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        Returns:
            (List[my_ultralytics.engine.results.Results]): A list of prediction results, encapsulated in the Results class.
        """
    ...
    def _get_hub_session(model: str):
        ...
        from my_ultralytics.hub.session import HUBTrainingSession
    ...
    def is_hub_model(model: str) -> bool:
        ...
        return any(
            (
                model.startswith(f"{HUB_WEB_ROOT}/models/"),  # i.e. https://hub.my_ultralytics.com/models/MODEL_ID
                [len(x) for x in model.split("_")] == [42, 20],  # APIKEY_MODEL
                len(model) == 20 and not Path(model).exists() and all(x not in model for x in "./\\"),  # MODEL
            )
        )

    def _new(self, cfg: str, task=None, model=None, verbose=False, device=None) -> None:
        """
        ...
        #修改的
        # device = self.overrides.get("device")
        # self.model = (model or self._smart_load("model"))(cfg_dict, verbose=verbose and RANK == -1)  # build model
        #修改的
        self.model = (model or self._smart_load("model"))(cfg_dict, verbose=verbose and RANK == -1, device=device)  # build model
        self.overrides["model"] = self.cfg
        self.overrides["task"] = self.task
        if device:
            self.overrides["device"] = device
    ...
    def reset_weights(self) -> "Model":
        ...
        Returns:
            self (my_ultralytics.engine.model.Model): The instance of the class with reset weights.
    ...
    def load(self, weights: Union[str, Path] = "yolov8n.pt") -> "Model":
        ...
        Returns:
            self (my_ultralytics.engine.model.Model): The instance of the class with loaded weights.
    ...
    def save(self, filename: Union[str, Path] = "saved_model.pt", use_dill=True) -> None:
        ...
        """
        self._check_is_pytorch_model()
        from my_ultralytics import __version__
        from datetime import datetime

        updates = {
            "date": datetime.now().isoformat(),
            "version": __version__,
            "license": "AGPL-3.0 License (https://my_ultralytics.com/license)",
            "docs": "https://docs.my_ultralytics.com",
        }
    ...
    def predict(
        ...
        **kwargs,
        ...
        is_cli = (sys.argv[0].endswith("yolo") or sys.argv[0].endswith("my_ultralytics")) and any(
    ...
    def track(
        ...
        **kwargs,
        ...
        Returns:
            (List[my_ultralytics.engine.results.Results]): A list of tracking results, encapsulated in the Results class.

        Raises:
            AttributeError: If the predictor does not have registered trackers.
        """
        if not hasattr(self.predictor, "trackers"):
            from my_ultralytics.trackers import register_tracker
    ...
    def benchmark(
        ...
        **kwargs,
        ...
        It uses the 'benchmark' function from the my_ultralytics.utils.benchmarks module. The benchmarking is configured
        ...
        """
        self._check_is_pytorch_model()
        from my_ultralytics.utils.benchmarks import benchmark
    ...
    def train(
        ...
        **kwargs,
        ...
        different scenarios, such as resuming training from a checkpoint, integrating with my_Ultralytics HUB, and
        updating model and configuration after training.

        When using my_Ultralytics HUB, if the session already has a loaded model, the method prioritizes HUB training
        ...
        """
        self._check_is_pytorch_model()
        if hasattr(self.session, "model") and self.session.model.id:  # my_Ultralytics HUB session with loaded model
            if any(kwargs):
    ...
    def tune(
        ...
        **kwargs,
        ...
        When Ray Tune is enabled, it leverages the 'run_ray_tune' function from the my_ultralytics.utils.tuner module.
        ...
        self._check_is_pytorch_model()
        if use_ray:
            from my_ultralytics.utils.tuner import run_ray_tune
    ...
    def names(self) -> list:
        ...
        using the 'check_class_names' function from the my_ultralytics.nn.autobackend module.

        Returns:
            (list | None): The class names of the model if available, otherwise None.
        """
        from my_ultralytics.nn.autobackend import check_class_names
